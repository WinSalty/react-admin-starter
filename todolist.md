...
阶段 8 已完成。已支持多级动态菜单和菜单扩展字段，侧边栏支持深层选中和展开状态持久化；右上角用户区已改为下拉面板并融合退出登录；非工作台页面已接入紧凑 `PageHeader`；登录和注册页已升级为品牌化认证布局。已通过 `npm run typecheck` 和 `npm run build` 验证。
...
1. 进入阶段 9 前，先重新调整登录和注册页配色，使认证页视觉与后台系统主界面保持一致。
2. 进入阶段 9，扩展系统管理基础模块。
3. 优先实现用户管理、角色管理、菜单管理、字典管理、日志管理的页面模板和 service/mock 数据链路。
4. 保持系统管理模块和现有查询管理模板一致的筛选、分页、详情、新增/编辑和状态处理规范。
