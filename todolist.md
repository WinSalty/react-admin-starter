...
1. 对接真实后端接口，替换所有 mock 数据调用。
2. 根据实际业务需求扩展更多管理模块。
3. 完善端到端测试覆盖率。
4. 适配邮箱注册流程：注册页增加邮箱验证码输入与发送验证码按钮，`src/services/auth.ts` 的注册参数和调用链路需对齐后端 `/api/auth/register/verify-code` 与 `verifyCode` 字段。
...
- 2026-04-18：记录后续适配项：后端注册流程已增加邮箱验证码，前端注册页后续需补充验证码输入、发送验证码按钮与 auth service 参数调整。claude-opus-4-7
