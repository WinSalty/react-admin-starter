...
- [ ] 设计并实现多级动态菜单模型，支持目录、菜单、隐藏路由、外链、禁用、徽标、重定向等扩展字段。
- [ ] 调整动态菜单 mapper 和过滤逻辑，支持父级目录无页面路径、深层菜单展示、深层路由选中。
- [ ] 优化侧边栏展开逻辑，支持默认展开父级目录、切换页面后保持展开状态。
- [ ] 重构右上角用户区，头像点击后展示登录账号、角色、当前时间，将退出登录融合进下拉面板。
- [ ] 制定并实现页面头部规范，压缩非工作台页面顶部空间，统一标题、描述和操作区布局。
- [ ] 美化登录和注册页面，升级为品牌化认证布局，兼顾桌面端和移动端。
- [ ] 更新 README 中的菜单模型、页面头部规范和认证页说明。
- [ ] 运行 `npm run typecheck` 和 `npm run build` 验证。

### 阶段 9：系统管理基础模块

- [ ] 用户管理：实现用户列表、筛选、详情、新增/编辑、状态切换、重置密码占位、分配角色入口。
- [ ] 角色管理：实现角色列表、详情、新增/编辑、状态切换、权限分配入口。
- [ ] 菜单管理：实现多级菜单树、菜单类型、图标、排序、路由路径、权限编码维护模板。
- [ ] 字典管理：实现字典类型、字典项、状态、排序和缓存刷新入口。
- [ ] 日志管理：实现登录日志、操作日志、接口日志列表和筛选模板。
- [ ] 所有系统管理模块数据通过 service 获取，mock 数据统一放入 `src/mocks`。
- [ ] 更新 README 系统管理模块说明。
- [ ] 运行 `npm run typecheck` 和 `npm run build` 验证。
...
- [ ] 梳理菜单、路由、按钮权限编码命名规范。
- [ ] 实现角色维度权限分配 UI，占位支持菜单、路由、按钮权限树。
- [ ] 建立后端动态菜单到本地页面组件的稳定映射表。
- [ ] 增加系统配置页面模板，支持基础参数、开关配置和前端缓存配置入口。
- [ ] 更新 README 权限分配和系统配置说明。
- [ ] 运行 `npm run typecheck` 和 `npm run build` 验证。

### 阶段 11：质量保障

- [ ] 补充核心工具函数单元测试方案。
- [ ] 补充登录、菜单权限、查询列表的端到端冒烟测试方案。
- [ ] 增加错误边界和页面级异常兜底。
- [ ] 梳理真实后端接入 checklist。
- [ ] 更新 README 测试和接入说明。
- [ ] 运行最终验证并整理提交。
...
阶段 7 已完成。README 已补充主题定制、服务层替换真实 API、权限接口字段映射和构建拆包说明；路由已改为懒加载，Vite 已拆分 `react`、`antd`、`echarts` vendor chunk。当前已根据新的产品方向补充阶段 8-11 开发规划。
...
1. 进入阶段 8，优先升级框架体验。
2. 实现多级动态菜单扩展、右上角用户下拉面板、页面头部规范和登录注册页美化。
3. 阶段 8 完成后，再进入阶段 9 扩展用户管理、日志管理、菜单管理、字典管理等系统管理模块。
