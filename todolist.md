...
2026-04-18：已新增公告管理页面阶段计划，当前已接入公告管理页面、路由、菜单与工作台公告提醒，邮箱注册页适配仍待继续。
...
1. 适配邮箱注册流程：注册页增加邮箱验证码输入与发送验证码按钮，`src/services/auth.ts` 的注册参数和调用链路需对齐后端 `/api/auth/register/verify-code` 与 `verifyCode` 字段。
2. 对接真实后端接口，替换剩余 mock 数据调用。
3. 根据实际业务需求扩展更多管理模块。
4. 完善端到端测试覆盖率。
...
- 2026-04-18：完成公告管理页面接入，新增 notice 类型与 service、公告管理页面、路由、菜单和工作台公告提醒入口。claude-opus-4-7
